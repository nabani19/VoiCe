...
type PhraseTuple = [RegExp, string, number];
type IntentTuple = [string, RegExp];
...
export function detectTone(text) {
  ...
  for (const [re, tone, w] of PHRASES) {
    if (re.test(text)) (scores as Record<string, number>)[tone] += w;
  }
  for (const ch of text.match(/\p{Extended_Pictographic}\uFE0F?/gu) || []) {
    const emojiTone = (EMOJI as Record<string, string>)[ch];
    if (emojiTone) (scores as Record<string, number>)[emojiTone] += 1.5;
  }
...
function pick(list: string[], last: string | null, seed?: number): string {
...
export function generateReply(text: string, opts: ReplyOptions = {}) {
  const msg = String(text || "").trim();
  if (!msg) {
    return {
      tone: "neutral", confidence: 0, intent: "statement", lang: "en",
      natural: "", funny: "", flirty: "", confident: "",
      candidates: [] as Array<{ id: string; body: string; style: string }>, scores: {}
    };
  }
  ...
  const langBank = (T as Record<string, any>)[lang] || (T as Record<string, any>)["en"];
  const bank = useIntent ? (langBank[intent] || langBank["neutral"]) : (langBank[tone] || langBank["neutral"]);
